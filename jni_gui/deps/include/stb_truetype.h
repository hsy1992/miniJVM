...
static int stbtt__close_shape_rename(stbtt_vertex *vertices, int num_vertices, int was_off, int start_off,
   ...
   *pvertices = NULL;
   ...
   if (numberOfContours > 0) {
      ...
      for (i=0; i < n; ++i) {
         ...
         if (next_move == i) {
            if (i != 0)
               num_vertices = stbtt__close_shape_rename(vertices, num_vertices, was_off, start_off, sx,sy,scx,scy,cx,cy);
