...
static bool _glfmEGLInit(GLFMPlatformData *platformData) {
    ...
    while (true) {
        ...
        eglChooseConfig(platformData->eglDisplay, attribs, &platformData->eglConfig, 1,
                        &numConfigs);
        if (numConfigs) {
            // Found!
...
jobject copyAssets(JNIEnv *env, jobject jassetMgr, char *cpath) {
    ...
    for (int i = 0; i < stringCount; i++) {
        ...
        strcat(assetpath, cpath);
        if (strlen(cpath) != 0)strcat(assetpath, "/");
        strcat(assetpath, rawString);

        char expath[512];
        memset(expath, 0, 512);
        strcat(expath, exDirRoot);
        strcat(expath, "/");
 ...
 * @return
 */
static int copyFile2ExternData(struct android_app *app) {
    const char *expath = glfmGetResRoot();
    LOG_DEBUG("expath=%s\n", expath);
