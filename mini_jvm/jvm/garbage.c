    ...
    while (NULL != (op = (GarbageOp *) linkedlist_pop_end(collector->operation_cache))) {
        switch (op->op_type) {
            case GARBAGE_OP_REG: {
                ...
                jvm_printf("R: %s[0x%llx] \n", utf8_cstr(sus), (s64) (long) op->val);
//                if (mb->type == MEM_TYPE_INS && utf8_equals_c(mb->clazz->name, "java/lang/String")) {
//                    utf8_clear(sus);
//                    garbage_thread_unlock();
//                    jstring_2_utf8((Instance *) mb, sus);
//                    garbage_thread_lock();
//                    jvm_printf("%s\n", utf8_cstr(sus));
//                }
                utf8_destory(sus);
            ...
            case GARBAGE_OP_HOLD: {
                hashset_put(collector->objs_holder, op->val);

#if _JVM_DEBUG_GARBAGE_DUMP
                Utf8String *sus = utf8_create();
                getMemBlockName((MemoryBlock *) op->val, sus);
                jvm_printf("+: %s[0x%llx] \n", utf8_cstr(sus), (s64) (long) op->val);
                utf8_destory(sus);
#endif
                break;
            ...
            case GARBAGE_OP_RELEASE: {
                hashset_remove(collector->objs_holder, op->val, 0);
#if _JVM_DEBUG_GARBAGE_DUMP
                Utf8String *sus = utf8_create();
                getMemBlockName((MemoryBlock *) op->val, sus);
                jvm_printf("-: %s[0x%llx] \n", utf8_cstr(sus), (s64) (long) op->val);
...
//    time_startAt = currentTimeMillis();
    ...
    for (; hashset_iter_has_more(&hti);) {
        ...
        if (mb->garbage_mark != collector->flag_refer) {
            hashset_remove(collector->objs, mb, 0);
...
void garbage_mark_object(__refer ref) {
    if (ref) {
        ...
        if (__garbage_find_obj(obj) == NULL) {
            int debug = 1;
        } else
